...
export default function JobDetailPage() {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setHasApplied(true);
        analytics.applicationTracked("Applied");
        ...
        <div className="agent-card p-6 animate-fade-up">
          ...
          {lowConfidence && (
            <div className="mt-3 flex items-start gap-2 rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-2">
              ...
              <p className="text-xs text-amber-400/80">
                <span className="font-semibold text-amber-400">Low confidence</span> — this score is based on limited signals (few detectable skills in your CV, or a job advert with little extractable detail). Treat it as a rough guide, not a verdict.
              </p>
            </div>
          )}
          ...
          <div className="mt-3 pt-3 border-t border-zinc-800/30 flex items-center gap-3">
            ...
            <div className="ml-auto flex items-center gap-2">
              <button
                onClick={() => {
                  const text = `I just scored ${fitScore}% fit for ${job?.title} at ${job?.companyName} on CareerOS 🎯\n\nCheck your own fit: https://careeros.live`;
              ...
              <button
                onClick={() => {
                  const text = `Just scored ${fitScore}% fit for ${job?.title} at ${job?.companyName} on @CareerOS_live 🎯\n\nCheck your own CV fit:`;
                  const url = "https://careeros.live";
                  window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(url)}`, "_blank", "noopener,noreferrer");
                }}
                className="flex items-center gap-1 text-xs text-zinc-600 hover:text-sky-400 transition-colors"
      ...
      <div className="agent-card p-6 animate-fade-up">
        <div className="flex gap-3">
          <button
            onClick={handleApply}
            disabled={applying}
            className={`flex-1 text-center justify-center ${hasApplied ? "agent-button" : "agent-button-primary"}`}
          >
            {applying ? (
              <>
          ...
          {hasApplied && (
            <Link href="/applications" className="agent-button px-4 justify-center">
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {!hasApplied && (
          <p className="text-xs text-zinc-600 text-center mt-2">
            Opens the company site and tracks your application automatically.
          </p>
        )}
      </div>
