...
export default function PricingPage() {
  ...
  const [momoChannel, setMomoChannel] = useState(1);
  ...
  const handleMomoCharge = async (otpcode?: string) => {
    ...
    setMomoLoading(true);
    try {
      const res = await fetch("/api/payment/momo-charge", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          plan: annual ? "annual" : "monthly",
  ...
  const handleMomoConfirm = async () => {
    ...
    try {
      const res = await fetch("/api/payment/verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ref: momoRef }),
      });
      const d = await res.json();
      if (d.isPremium) {
        posthog?.capture("premium_activated", { method: "momo_ussd" });
        setIsPremium(true);
        toast.success("Premium activated!");
      } else {
        toast.error(d.message || "Payment not confirmed yet. Approve the prompt, then try again.");
      }
    } catch {
      toast.error("Verification failed. Please try again.");
    } finally {
          ...
          <div className="rounded-2xl border border-purple-500/30 bg-[#0d0d18] overflow-hidden flex flex-col flex-1 relative">
            ...
            {isPremium ? (
              ...
              <>
                ...
                <button
                  onClick={() => setMomoOpen((v) => !v)}
                  className="w-full mt-3 rounded-xl border border-white/[0.08] bg-white/[0.03] px-4 py-3 mono text-xs text-zinc-400 hover:text-zinc-200 hover:bg-white/[0.05] transition-all"
                >
                  {momoOpen ? "Hide MoMo checkout" : "Or pay with MoMo — approve on your phone, no redirect"}
                </button>
                    ...
                    {momoOtpRequired && !momoRef ? (
                      ...
                      <>
                        <p className="text-sm text-zinc-300">
                          ...
                          Approve it on your phone, then confirm below.
                        </p>
                        <button
                          onClick={handleMomoConfirm}
                          disabled={momoVerifying}
                          className="agent-button-primary w-full justify-center py-3 text-sm font-bold press-scale disabled:opacity-50"
                        >
                          {momoVerifying ? (
                            <>
                      ...
                      <>
                        <div className="grid grid-cols-3 gap-2">
                          {[
                            ...
                            <button
                              key={n.id}
                              onClick={() => setMomoChannel(n.id)}
                              className={`rounded-lg px-2 py-2 mono text-xs transition-all border ${
                                momoChannel === n.id
                        ...
                        <input
                          type="tel"
                          value={momoPhone}
                          onChange={(e) => setMomoPhone(e.target.value.trim())}
                        ...
                        >
                          ...
                          ) : `Send payment prompt — GHS ${annual ? "199" : "25"}`}
                        </button>
                      </>
                    )}
                  </div>
