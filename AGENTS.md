...
- Disbursements: referral conversions credit GHS 5 to `User.earningsBalance` (no transfer); users withdraw the accumulated balance on demand from /referrals (min GHS 5) — `src/lib/referral-reward.ts` `requestWithdrawal`, atomic deduct-then-transfer with refund on failure, history in `Withdrawal` model
